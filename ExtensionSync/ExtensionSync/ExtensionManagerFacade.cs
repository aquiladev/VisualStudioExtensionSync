...
namespace ExtensionSync
{
	public class ExtensionManagerFacade
	{
		...
		public List<ExtensionInformation> GetInstalledExtensionsInformation()
		{
			...
			return userExtensions.Select(e => new ExtensionInformation
					{
						Name = e.Header.Name,
		...
		private void QueryExecuteCompleted(object sender, ExecuteCompletedEventArgs e)
		{
			try
			{
				...
				var entry =
					e.Results.Cast<VSGalleryEntry>()
					.SingleOrDefault(r => r.Name == extensionInformation.Name
						&& r.VsixID == extensionInformation.Identifier);
				if (entry == null)
				{
				...
				var installedExtension = installedExtensions
					.FirstOrDefault(ext => ext.Name == extensionName
									&& ext.Identifier == extensionInformation.Identifier);
				if (AutoUpdateExtensions)
				{
					if (installedExtension != null 
						&& installedExtension.Version.ToString() == entry.VsixVersion)
					{
						return;
					}
				}
